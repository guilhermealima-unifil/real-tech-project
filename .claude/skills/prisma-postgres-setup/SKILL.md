...
When presenting choices to the user (region selection, project deletion, etc.), use Claude Code's available interactive selection mechanism. If none is available, ask one concise plain-text question. Do not assume a particular tool name is always available.
