...
When presenting choices to the user (region selection, project deletion, etc.), use the interactive selection mechanism available in the current agent. If none is available, ask one concise plain-text question. Do not assume a tool named `ask` exists.
